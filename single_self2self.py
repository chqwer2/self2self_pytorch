...
transforms = {
    "train": A.Compose([
                ...
                A.HueSaturationValue(hue_shift_limit=5, sat_shift_limit=5, val_shift_limit=0, always_apply=False, p=0.5) ], p=0.3),
            ], p=1.0),
                # A.ToTensorV2(),
    ], p=1.0)
}
...
def train_self2self(imgdir, img_channel, p, sigma=-1, is_realnoisy = False):
    ...
    # =========== Prepare
    cfg.imgdir = os.path.join(cfg.imgdir, imgdir.split('/')[-1] + '-DA')
    ...
    for itr in range(cfg.iteration):
        ...
        img_input = transforms['train'](image=img)['image']
        #y = img - img_input
        y = img
        ...
        img_input_tensor = image_loader(img_input, device, p1, p2, degree).cuda()
        y = image_loader(y, device, p1, p2, degree).cuda()
  
        mask = np.expand_dims(np.transpose(mask,[2,0,1]), 0)
        mask = torch.tensor(mask).to(device, dtype=torch.float32).cuda()
        
        Gaussian_Noise = torch.tensor(np.random.normal(scale=50 / 255., size=img_input_tensor.shape).astype(np.float32)).to(device, dtype=torch.float32).cuda()
        # print("Noise shape:", Gaussian_Noise.shape, img_input_tensor.shape, mask.shape)
        model.train()
        img_input_tensor = img_input_tensor + mask*Gaussian_Noise
        ...
        #break   
        if (itr+1)%1000 == 0:
        # if ((itr+1)%2500 == 0 and (itr+1)<100000 == 0) or (((itr+1)<5000 and (itr+1)%200 == 0)) or (((itr+1)<100 and (itr+1)%10 == 0)):
            model.eval()
